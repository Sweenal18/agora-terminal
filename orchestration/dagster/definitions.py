...
all_assets = load_assets_from_modules([fundamentals, equity, macro, data_quality, cdc])
...
data_quality_job = define_asset_job(
    ...
    selection=["silver_data_quality", "gold_data_quality"],
)
cdc_job = define_asset_job(
    name="cdc_instruments",
    selection=["silver_instruments_cdc"],
)
...
data_quality_schedule = ScheduleDefinition(
    ...
    cron_schedule="0 3 * * *",
    name="data_quality_daily_schedule",
)
cdc_schedule = ScheduleDefinition(
    job=cdc_job,
    cron_schedule="*/15 * * * *",
    name="cdc_instruments_schedule",
)
defs = Definitions(
    assets=all_assets,
    jobs=[fundamentals_job, macro_job, equity_job, data_quality_job, cdc_job],
    schedules=[fundamentals_schedule, macro_schedule, data_quality_schedule, cdc_schedule],
)